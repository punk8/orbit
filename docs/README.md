# Orbit Design Docs

This directory captures the pre-development design baseline for Orbit.

Read in this order:

1. [Product Principles](./product-principles.md)
2. [Architecture](./architecture.md)
3. [Data Model](./data-model.md)
4. [Privacy and Permissions](./privacy-permissions.md)
5. [UI Design](./ui-design.md)
6. [Milestone Plan](./milestone-plan.md)
7. [Implementation Plan](./implementation-plan.md)
8. [Handoff Pack](./handoff-pack.md)
9. [Perception Research Spike](./perception-research-spike.md)
10. [Development Tasks](./development-tasks.md)
11. [Alpha Runtime Decisions](./alpha-runtime-decisions.md)
12. [Alpha Product Closure Spec](./alpha-product-closure-spec.md)
13. [Sanitized Real Fixtures Guide](./sanitized-real-fixtures-guide.md)
14. [Alpha Test Strategy](./alpha-test-strategy.md)
15. [Privacy Todos](./privacy-todos.md)
16. [Alpha Hardening Goals](./alpha-hardening-goals.md)
17. [Alpha Release Checklist](./alpha-release-checklist.md)
18. [Handoff: Next AI Provider Work](./handoff-next-ai-provider.md)
19. [Alpha UI Implementation Spec](./alpha-ui-implementation-spec.md)
20. [Complete Product Spec](./complete-product-spec.md)
21. [Background Observation Core Spec](./background-observation-core-spec.md)
22. [Background Observation Implementation Plan](./background-observation-implementation-plan.md)
23. [Source Adapter Complete Contract](./source-adapter-complete-contract.md)
24. [Semantic Quality And Evaluation](./semantic-quality-evaluation.md)
25. [Release Readiness Spec](./release-readiness-spec.md)
26. [Alpha Perception And Context Completion](./alpha-perception-and-context-completion.md)
27. [LLM Perception And Context Automation](./llm-perception-and-context-automation-plan.md)
28. [Goal 9D Live Perception Runtime Blocker](./goal-9d-live-perception-runtime-blocker.md)

The design follows the project-level guidance in [AGENTS.md](../AGENTS.md):

`Source Adapter -> Event -> Activity Session -> Knowledge Artifact -> Memory -> Recommendation`

Use the Alpha documents to understand the current implementation boundary. Use the complete-product
documents to plan work that must hold up for a full user-installable product.
