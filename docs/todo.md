...
- 2026-05-28 已收敛：Review Queue 改为专用审阅工作台布局，顶部状态条全宽展示待审 Knowledge / Memory 计数和隐私边界，待审卡片锚定主列；只有 Knowledge 或 Memory 一类待审内容时自动单列，避免首屏卡片漂到右侧。证据展开按钮降级为轻量工具按钮，让确认、拒绝、归档保持主操作位。
- 2026-05-28 checkpoint 验收：`pnpm typecheck`、`pnpm test`（48 files / 265 tests）、`pnpm lint`、`pnpm --filter @orbit/desktop build`、`pnpm --filter @orbit/desktop package:dir`、`pnpm --filter @orbit/desktop package:smoke -- --app release/mac-arm64/Orbit.app --orbit-home /tmp/orbit-package-smoke-review-queue-layout` 均已通过；package smoke 和 release 内容扫描未发现 `.db` / `.sqlite` / `.jsonl` / autoresearch / `research-results.tsv` / `fixtures` / `.tmp` / `perception-sidecars` 混入 `apps/desktop/release/mac-arm64/Orbit.app`。
- 2026-05-28 已收敛：Handoff 在尚未生成交接包时新增自助工作流，说明 Agent-safe 预览边界，并提供生成今天交接、打开 Review Queue、打开 Activity、打开 Sources 的本地路径；普通用户从空 Handoff 页面即可知道先审阅、追证据、补来源或直接生成 Today 包，且不会引入外部副作用或 raw/未确认内容导出。
- 2026-05-28 checkpoint 验收：`pnpm typecheck`、`pnpm test`（48 files / 266 tests）、`pnpm lint`、`pnpm --filter @orbit/desktop build`、`pnpm --filter @orbit/desktop package:dir`、`pnpm --filter @orbit/desktop package:smoke -- --app release/mac-arm64/Orbit.app --orbit-home /tmp/orbit-package-smoke-handoff-empty-workflow` 均已通过；package smoke 和 release 内容扫描未发现 `.db` / `.sqlite` / `.jsonl` / autoresearch / `research-results.tsv` / `fixtures` / `.tmp` / `perception-sidecars` 混入 `apps/desktop/release/mac-arm64/Orbit.app`；`git diff --check` 通过。
