...
它不是 Codex 或 SeaTalk 的附加工具。Codex 与 SeaTalk 只是早期数据源。长期方向是成为一个外部工作助手：持续理解用户的工作上下文，沉淀可检索的记忆，并把零散事件整理成有用的总结、洞察和行动建议。
...
- **数据源会变化，Event schema 是稳定层**: Codex、SeaTalk、屏幕录制、日历、邮件、Jira、GitLab 都只是 source adapter；核心逻辑必须围绕统一事件模型、处理流水线和记忆层设计。
...
- **Codex**: 工程会话、代码改动、命令执行、测试结果、提交记录、任务过程和结论。
- **SeaTalk**: 聊天消息、未读、@我、私聊、群讨论、自动值班事件和待确认回复。

后续可能接入：

- **屏幕录制 / 屏幕理解**: 在用户授权后，通过录制或采样屏幕内容分析工作流、操作过程和上下文变化。
- **日历 / 邮件 / 文档 / Jira / GitLab**: 用于补全工作计划、沟通记录、需求背景和交付状态。
- **本地文件系统**: 读取用户明确授权的项目目录、笔记和输出文档。
...
- **来源可扩展**: Codex、SeaTalk、屏幕录制等都应抽象为 source adapter，不让核心逻辑绑定具体应用。
...
- **本地优先但接口开放**: 默认在本地采集、处理和存储；同时提供 CLI、MCP、Skill 或本地 HTTP API，让 Codex、Claude Code 等外部 Agent 能在授权范围内读取上下文。
...
- **Source Adapter**: 连接外部数据源的适配层，如 Codex adapter、SeaTalk adapter、Screen adapter。

## 架构方向

Orbit 的长期架构应围绕以下稳定层推进：

1. **Source Adapter**: 负责接入 Codex、SeaTalk、Screen、Calendar、Mail、Docs、Jira、GitLab、本地文件等来源，只做采集、权限声明和来源元数据标注。
...
1. **Goal 1: Local Data Spine**: 完成 Task 1-5，建立 pnpm monorepo、核心类型、SQLite、本地 fixtures、fixture ingestion 和基础 CLI。该阶段不得实现完整 Electron UI、真实 Codex/SeaTalk adapter、外部 AI 或读取私人本地数据。
...
3. **Goal 3: Product Shell And Real Sources**: 完成 Task 11-13，建立 Electron shell、真实 Codex 只读 adapter，以及 SeaTalk 安全接入决策。SeaTalk 如果没有明确安全 read path，只保留 fixture-backed adapter 并记录 blocker。
