...
- **优先补真实工作现场入口**: 为了快速验证 Orbit 是否真的能理解用户工作，不要长期只打磨底座、fixtures 或控制面。当前产品优先级应前移到最小真实 Screen/OCR 能力：在用户显式触发或授权后，采集当前屏幕/窗口的低频截图，使用本地 OCR 提取中英文文本，生成可追溯 Event，并进入 Activity、Knowledge、Memory、Handoff 链路。它不是产品中心，也不能默认后台录屏，但它是理解用户工作现场和对标 Yansu 类连续性产品的关键入口。
...
- **不要做成截图搜索工具，但必须能看见工作现场**: 屏幕录制、OCR、时间线和回放只是能力之一；Orbit 的产品中心是工作事件理解、长期记忆、上下文总结和可解释建议。不过在 Alpha 阶段，最小真实 Screen/OCR 是优先级很高的功能证伪切片，因为没有它，Orbit 很难从真实桌面工作中获得足够上下文。
...
7. 优先实现手动触发、显式授权、低频、summary-first 的真实 Screen/OCR 垂直切片，用来验证 Orbit 能从真实屏幕生成 Event/Activity/Knowledge/Handoff；再扩展到后台常驻、音频和会议转写。
8. 屏幕帧、OCR、音频和会议转写必须在权限、可见运行状态、暂停/停止、受保护应用、短 TTL、redaction、审计和资源预算完整后再启用；不得默认开启持续录屏或麦克风采集。
